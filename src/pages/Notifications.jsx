        ...
        <main className="grow px-3 md:px-4 lg:px-6 xl:px-8 py-8">
          <div className="max-w-400 mx-auto flex flex-col gap-6">
            ...
            <div className="mb-6">
              <div className="flex border-b border-slate-200 dark:border-zinc-800 gap-8">
                <button
                  ...
                  className={`flex items-center gap-2 pb-3 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === 'all'
                ...
                <button
                  ...
                  className={`flex items-center gap-2 pb-3 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === 'order'
                    ? 'border-[#135bec] text-[#135bec]'
                    : 'border-transparent text-slate-500 dark:text-zinc-400 hover:text-slate-800 dark:hover:text-zinc-200'
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 pb-3 px-2 font-bold text-sm border-b-2 transition-colors ${activeTab === 'promotion'
                    ? 'border-[#135bec] text-[#135bec]'
                    : 'border-transparent text-slate-500 dark:text-zinc-400 hover:text-slate-800 dark:hover:text-zinc-200'
                    }`}
                >
                  ...
                  <div className="space-y-3">
                    {todayNotifications.map((notification) => (
                      <div
                        ...
                        className={`flex gap-4 p-4 rounded-xl border transition-all cursor-pointer group relative overflow-hidden ${notification.isRead
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-1">
                            ...
                            {!notification.isRead && (
                              <span className="text-[11px] font-medium text-[#135bec] bg-[#13a4ec]/10 px-2 py-0.5 rounded-full uppercase tracking-tighter">Mới</span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3">
                            {notification.action && (
                              <button
                                ...
                                className={`text-xs font-bold transition-all ${notification.action === 'Mua ngay' ? 'px-4 py-1.5 bg-[#135bec] hover:bg-[#135bec]/90 text-white rounded' : 'text-[#135bec] hover:underline'}`}
                              >
                                {notification.action}
                              </button>
                            )}
                        ...
                        {!notification.isRead && (
                          <div className="shrink-0 flex items-center">
                            <div className="size-2.5 rounded-full bg-[#135bec] shadow-[0_0_8px_rgba(19,164,236,0.6)]"></div>
                          </div>
                        )}
                      </div>
                    ))}
            ...
            <div className="mt-12 pt-8 border-t border-slate-200 dark:border-zinc-800 flex justify-center gap-4">
              <Link to="/" className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-[#135bec] transition-colors">
                <span className="material-symbols-outlined text-lg">arrow_back</span>
                Quay lại Trang chủ
              </Link>
              <span className="text-slate-300 dark:text-zinc-700">|</span>
              <Link to="/profile" className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-[#135bec] transition-colors">
                Hồ sơ cá nhân
